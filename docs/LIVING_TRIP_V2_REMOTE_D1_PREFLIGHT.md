...
The existing `jotrip-home` audit bridge can continue **read-only schema checks** using its working user-owned token without transferring credentials. Full production backup still needs an access-restricted durable destination and a tested restore; an ephemeral CI schema export is not a customer-data backup. The `jotrip-trip` diagnostic now performs token verification and D1 identity checks automatically, with schema-only export available on manual workflow dispatch to avoid unnecessary repeated exports.
