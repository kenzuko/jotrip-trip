...
- `DELETE /api/trip/session`: after two-step UI confirmation, deletes only the V2 trip context and transcript. A short-lived tombstone blocks an old browser tab from recreating the deleted session. Separately consented booking leads are unaffected.
...
The `/api/health` readiness check requires all three V2 tables: sessions, turns and deletion tombstones. Back up and inspect remote D1 before applying migration 0009. CI exercises an actual local D1 SQL fixture and mocked Worker tests, but has not migrated or tested the remote database. Cross-device recovery needs explicit account linking or another authorized transfer mechanism. Saving dates and a planning canvas does not mean inventory is reserved or booking is confirmed.
