...
- `DELETE /api/trip/session`: after two-step UI confirmation, deletes only the V2 trip context and transcript. A short-lived tombstone blocks an old browser tab from recreating the deleted session. Separately consented booking leads are unaffected. Booking handoff now sends only contact plus explicit consent from the browser; the Worker derives a minimal summary from canonical server state and stores a consent version/time under migration 0010. Staff-only erasure requires a separate lead-admin secret and an independently verified customer request.
